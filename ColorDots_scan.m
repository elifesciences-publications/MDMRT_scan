function ColorDots_scan(subjid,test_comp,exp_init,eye,scan,run,task_order, button_order,subkbid,expkbid,triggerkbid)
...
fid1=fopen(['Output/' subjid '_dots_task_run_' num2str(run) '_' timestamp '.txt'], 'a');
...
save(['Output/' subjid '_dots_task_run_' num2str(run) '_' timestamp '.mat'],'Dots','info')
...
if eye==1
    ...
    if dummymode==0
        movefile('recdata.edf',strcat('Output/', subjid,'_dots_task_run_', num2str(run), '_', timestamp,'.edf'));
    end;
end
...
elseif scan==0 && eye==0 && ((task_order==1 && run<3) || (task_order==2))
    CenterText(win,'We will continue to the next task shortly.', white,0,0);
else
